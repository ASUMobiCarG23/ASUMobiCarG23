...
//elarqam ely ana msaweha deh arqam elpins fel arduino
  ...
  const int motorLRF  = 50;  // Pin  2 of L293    omar lower right forward
  const int motorLRB = 60;  // Pin  7 of L293    omar lower right backward
  const int motorLLF  = 100; // Pin 10 of L293    omar lower left forward
  const int motorLLB = 90;  // Pin 14 of L293   omar lower left backward
...
void loop() {
  ...
  //If state is equal with letter 'G', car will go forward left
    	...
    	analogWrite(motorURF, vSpeed); analogWrite(motorLRF, vSpeed);  
        analogWrite(motorULF, vSpeed/2);    analogWrite(motorLLF, vSpeed/2); 
    }
  /**********************Forward Right************************/
  //If state is equal with letter 'I', car will go forward right
    else if (state == 'I') {
      
      	analogWrite(motorURF, vSpeed/2); analogWrite(motorLRF, vSpeed/2); 
        analogWrite(motorULF, vSpeed);      analogWrite(motorLLF, vSpeed); 
    }
  /***********************Backward****************************/
  ...
  //If state is equal with letter 'H', car will go backward left
    	...
    	analogWrite(motorULB,vSpeed/2);   analogWrite(motorLLB, vSpeed/2); 
        analogWrite(motorURB,vSpeed); analogWrite(motorLRB,vSpeed); 
    }
  /**********************Backward Right************************/
  //If state is equal with letter 'J', car will go backward right
    else if (state == 'J') {
    	analogWrite(motorURB,vSpeed/2);   analogWrite(motorLRB, vSpeed/2); 
        analogWrite(motorULB,vSpeed);   analogWrite(motorLLB,vSpeed); 
    }
  /***************************Left*****************************/
  //If state is equal with letter 'L', wheels will turn left
    else if (state == 'L') {
    	analogWrite(motorURF, vSpeed);   analogWrite(motorLRF, vSpeed); 
        analogWrite(motorLLB, vSpeed); analogWrite(motorULB,vSpeed); 
    }
  /***************************Right*****************************/
  //If state is equal with letter 'R', wheels will turn right
    else if (state == 'R') {
    	analogWrite(motorURB,vSpeed);   analogWrite(motorLRB,vSpeed); 
        analogWrite(motorLLF, vSpeed);   analogWrite(motorULF, vSpeed); 		
    }
  /************************Lights*****************************/
